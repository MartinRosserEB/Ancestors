...
use App\Entity\PersonMarryPerson;
use App\Form\PersonType;
use App\Form\FindPersonType;
use App\Form\LinkTwoPersonsType;
use App\Form\PersonMarryPersonType;
use App\Service\CheckAccess;
use App\Service\FindLink;
use App\Service\PreFill;
use App\Service\PrepareRelations;
...
class PersonController extends AbstractController
{
    ...
    {
        $oldestPerson = $this->getDoctrine()->getManager()->getRepository('App:Person')->findOldestPerson();
    ...
    public function linkForm(Request $request)
    {
        ...
        if ($form->isSubmitted() && $form->isValid()) {
            $em = $this->getDoctrine()->getManager();
            $person1 = $em->getRepository('App:Person')->findOneById($entity->getHusband());
            $person2 = $em->getRepository('App:Person')->findOneById($entity->getWife());
            if (!$person1 || !$person2) {
    ...
    public function findAction(Request $request)
    {
        ...
        $form->handleRequest($request);
        if ($form->isSubmitted() && $form->isValid()) {
            $result = $this->getDoctrine()->getManager()->getRepository('App:Person')->findPersonFromPartialInfoWithReadAccessCheck($entity, $this->getUser());
        }
    ...
    public function editMarriageAction(Request $request, Person $person1, Person $person2, CheckAccess $checkAccess)
    {
        ...
        $em = $this->getDoctrine()->getManager();
        $entity = $em->getRepository('App:PersonMarryPerson')
