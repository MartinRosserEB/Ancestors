...
class DisplayNode {
    ...
    public function recursivelySetSpouse($em)
    {
        ...
        if ($this->person->getFemale()) {
            $marriage = $em->getRepository('App:PersonMarryPerson')->findOneByWife($this->person->getId());
            if ($marriage) {
                $personId = $marriage->getHusband();
            }
        } else {
            $marriage = $em->getRepository('App:PersonMarryPerson')->findOneByHusband($this->person->getId());
            if ($marriage) {
                $personId = $marriage->getWife();
            }
        }
        if (isset($personId)) {
            $spouse = $em->getRepository('App:Person')->findOneById($personId);
            if ($spouse) {
                $this->spouse = $spouse;
            }
        }
